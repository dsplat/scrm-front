<template>
  <view class="learn-page">
    <NavBar :title="course?.title || '课程学习'" />
...
function goDetail() {
  uni.redirectTo({ url: `/pages/course/detail?id=${courseId.value}` })
}
...
async function handleFinish() {
  ...
  try {
    const res = await reportLearningProgress(courseId.value, current.value.chapter_id)
