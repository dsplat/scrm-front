      ...
      <view v-if="skus.length > 0" class="section-card">
        ...
        <view class="sku-list">
          <view
            ...
            :key="sku.sku_id"
            class="sku-item"
            :class="{ active: selectedSku?.sku_id === sku.sku_id, disabled: sku.stock <= 0 }"
            @tap="selectSku(sku)"
          >
            <text class="sku-name">
              {{ sku.name }}
            ...
            <text class="sku-meta">
              ...
              <template v-if="Number(sku.points_price) > 0">
                / {{ sku.points_price }}积分
              </template>
            </text>
            <text v-if="sku.stock <= 0" class="sku-soldout"> 已售罄 </text>
          </view>
        </view>
      </view>
...
async function load() {
  ...
  loading.value = true
  try {
    const res = await getShopProductDetail(productId.value)
