...
import { ProTable, type ColumnConfig, type SearchConfig, type ActionConfig, type RequestParams, type RequestResult } from '@/components/common/ProTable'
...
const columns: ColumnConfig[] = [
  ...
  { prop: 'status', label: '状态', width: 120, sortable: true, render: (row) => h(ElTag, { type: row.status === 'active' ? 'success' : 'info' }, () => row.status === 'active' ? '启用' : '停用') },
