...
async function loadData() {
  loading.value = true
  try {
    const res = (await http.get('/scrm/customer-groups')) as any
...
async function handleSubmit() {
  ...
  try {
    if (isEdit.value && editId.value) {
      await http.put(`/scrm/customer-groups/${editId.value}`, form.value)
      ElMessage.success('已更新')
    } else {
      await http.post('/scrm/customer-groups', form.value)
...
async function handleDelete(row: TableItem) {
  await ElMessageBox.confirm(`确定删除分组「${row.name}」？`)
  await http.delete(`/scrm/customer-groups/${row.id}`)
