...
async function loadData() {
  loading.value = true
  try {
    const res = (await http.get('/scrm/roles')) as any
...
async function handleSubmit() {
  ...
  try {
    if (isEdit.value && editId.value) {
      await http.put(`/scrm/roles/${editId.value}`, form.value)
      ElMessage.success('已更新')
    } else {
      await http.post('/scrm/roles', form.value)
...
async function handleDelete(row: TableItem) {
  await ElMessageBox.confirm(`确定删除角色「${row.name}」？`)
  await http.delete(`/scrm/roles/${row.id}`)
